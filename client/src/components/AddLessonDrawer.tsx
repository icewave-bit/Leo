...
export function AddLessonDrawer({ draft, defaultStudentId, onClose, onCreate }: AddLessonDrawerProps) {
  ...
  const whenLabel = useMemo(() => {
    const start = timeValueToHours(time);
    const end = start + durationMin / 60;
    return `${daysFull[day]}, ${dates[day]} · ${fmtTime(start)}–${fmtTime(end)}`;
  }, [day, dates, daysFull, time, durationMin]);
